        ...
        for concept_key, concept_data in data.items():
            if isinstance(concept_data, dict):
                ...
                node = Node(
                    ...
                    labels=["Concept"],  # Default rich label for concept mapping
                    session_id=f"{domain}_session",  # Assign to session
                    properties={**serialized_properties, "concept_id": concept_id}
                )
                ...
                if self.db.create_node(node):
                    ...
                    logger.error(f"Failed to create concept mapping node {concept_key}")
                ...
                for metaphor in concept_data.get("metaphors", []):
                    ...
                    metaphor_node = Node(
                        ...
                        labels=["Concept", "Metaphor"],  # Rich labels for metaphors
                    ...
                    if self.db.create_node(metaphor_node):
                        ...
                        edge = Edge(
                            ...
                            session_id=f"{domain}_session",  # Intrasession metaphor edge
                ...
                if source_concept and target_concept:
                    ...
                    if source_node_id and target_node_id:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create metaphorical connection: {e}")
    ...
    async def _import_nodes(self, nodes: List[Dict[str, Any]], domain: str, exploration_session: ExplorationSession) -> int:
        ...
        for node_data in nodes:
            ...
            if self.db.create_node(node):
                ...
                nodes_created += 1
                logger.debug(f"Created node with rich labels {original_labels}: {node_name}")
            else:
        ...
        for edge_data in edges:
            ...
            if source_id and target_id:
                ...
                if self.db.create_edge(edge):
                    edges_created += 1
                    logger.debug(f"Created intrasession relationship {original_type} between {source_id} and {target_id}")
                else:
                    logger.error(f"Failed to create relationship {original_type}")
                ...
                if concept1_id and concept2_id:
                    ...
                    if source_node_id and target_node_id:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create metaphor bridge: {e}")
    ...
    async def _import_concept_list(self, concepts: List[Dict[str, Any]], domain: str) -> int:
        ...
        for concept in concepts:
            ...
            if self.db.create_node(node):
                ...
                logger.error(f"Failed to create concept list node {concept_id}")
                ...
                for target_concept in connections:
                    ...
                    if source_node_id and target_node_id:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create connection mapping edge: {e}")
            ...
            if source_concept and target_concept:
                ...
                if source_node_id and target_node_id:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to create connection list edge: {e}")
